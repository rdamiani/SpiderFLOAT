...
class Material: #isotropic for the time being
    def __init__(self,**kwargs):
        ...
        prms={'matname':'ASTM992_steel','E':2.1e11,'nu':0.33, 'rho':8502., 'fy':345.e6, 'fyc':345.e6, 'fp':0.0, 'relx':0.0, 'eps_cu':0.0} #SI Units
        ...
        concrete={'matname':'Concrete',            'E':24.8e9,'nu':0.18,             'rho':2300,  'fy':0.,     'fyc':27.6e6, 'eps_cu':-.003} #SI Units
        ...
        tie={'matname':"Tie",                     'E':200e9, 'nu':0.33,             'rho':7805,  'fy':276.e6,  'fyc':276.e6} #SI Units;
        ...
        elif ((prms['matname'].lower() == 'ties') or (prms['matname'].lower() == 'hoop') or (prms['matname'].lower() == 'spiral')):
            for key in prms:
                if (not(key in kwargs) or (prms[key]==[])) and (key in strand):  #I need to operate on parameters not set by user
                    setattr(self,key,strand[key])
