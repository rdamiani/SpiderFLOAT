...
class Cable:
    ...
    def mass(self):
        """Dry Mass"""
        return self.tube.mass[0]  #dry mass
...
class LegCableBal(om.ImplicitComponent) :   
    ...
    def initialize(self):
        ...
        self.options.declare("Fxyz_tip", default=np.zeros(6), desc="Leg tip loads, forces and moments in global CS [N and Nm]" )
    def setup(self):
...
class cycle(om.Group):
    ...
    def setup(self):
        ...
        self.add_subsystem("uvwLC1", uvwProcessor(C1orC2=-1,D_L=parameters["D_L"],L_L0=parameters["L_L0"],Lh=parameters["Lhs"][0]),promotes_inputs=["uvw_legtip","tht_legtip"])
        self.add_subsystem("uvwLC2", uvwProcessor(C1orC2=+1,D_L=parameters["D_L"],L_L0=parameters["L_L0"],Lh=parameters["Lhs"][0]),promotes_inputs=["uvw_legtip","tht_legtip"])
        
        self.add_subsystem("uvwUC1", uvwProcessor(C1orC2=-1,D_L=parameters["D_L"],L_L0=parameters["L_L0"],Lh=parameters["Lhs"][1]),promotes_inputs=["uvw_legtip","tht_legtip"] )
        self.add_subsystem("uvwUC2", uvwProcessor(C1orC2=+1,D_L=parameters["D_L"],L_L0=parameters["L_L0"],Lh=parameters["Lhs"][1]),promotes_inputs=["uvw_legtip","tht_legtip"])

        self.add_subsystem("UC1", legCable(xyz0=parameters['UC1_xyz0'],material=parameters["UCmat"],sig00=parameters["sig00"][1],D_C00=parameters["D_C00"][1],z_legroot=parameters['z_legroot']))
        self.add_subsystem("LC1", legCable(xyz0=parameters['LC1_xyz0'],material=parameters["LCmat"],sig00=parameters["sig00"][0],D_C00=parameters["D_C00"][0],z_legroot=parameters['z_legroot']))
        #Symmetric cables provide symmetric geometry in mirrored y
        junk = np.copy(parameters["UC1_xyz0"])
        junk[:,1] *=-1
        self.add_subsystem("UC2", legCable(xyz0=junk,material=parameters["UCmat"],sig00=parameters["sig00"][1],D_C00=parameters["D_C00"][1],z_legroot=parameters['z_legroot']))
        junk = np.copy(parameters["LC1_xyz0"])
        junk[:,1] *=-1
        self.add_subsystem("LC2", legCable(xyz0=junk,material=parameters["LCmat"],sig00=parameters["sig00"][0],D_C00=parameters["D_C00"][0],z_legroot=parameters['z_legroot']))
        
        self.add_subsystem('LegCableBal',LegCableBal(parameters=parameters,Fxyz_tip=self.options['Fxyz_tip']),promotes_outputs=["uvw_legtip","tht_legtip"]) 
        ...
        self.connect("UC1.Bprel","LegCableBal.UC1_Bprel")
        self.connect("UC2.Bprel","LegCableBal.UC2_Bprel")
        self.connect("LC1.Bprel","LegCableBal.LC1_Bprel")
        ...
        self.nonlinear_solver.options['err_on_non_converge'] = False# debug
        self.linear_solver = om.DirectSolver()



class CableGroup(om.Group):
    """ Main Group doing the work on multiple parallel groups"""
    def initialize(self):
        self.options.declare("parameters")
    ...
    def setup(self):
        ...
        #Add subsystems
        #Inputs come through IndepVarComp
        ...
        #divide = divide_cases(n_DLCs, 5) 
        #for j, this_proc in enumerate(divide):
        for j in range(n_DLCs):
            #print("j=",j) #debug
            name = 'subcycle_%d' % j
            Fxyz_tip = parameters["Fxyz_tip"][j,:]
            sub = par.add_subsystem(name, cycle(parameters=parameters,Fxyz_tip=Fxyz_tip))  #here we are putting multile cycles, as many as DLCs
        
            #connections to cable components
            par_sys_name= "parallel.{:s}.".format(name)
            self.connect("D_Cs.D_C",[par_sys_name+"UC1.D",par_sys_name+"UC2.D"], src_indices=1)
            self.connect("D_Cs.D_C",[par_sys_name+"LC1.D",par_sys_name+"LC2.D"], src_indices=0)

            self.connect("sig0_Cs.sig0_C",[par_sys_name+"UC1.sig0",par_sys_name+"UC2.sig0"], src_indices=1)
...
def main(**kwargs):
    ...
    if len(opti):
        ...
        prob.model.add_objective("UC1LC1_mass.cables_mass", ref =10000.)#,  ref=5000.) #this means reducing structural mass
        
        #CONSTRAINTS  
        for j in range(parameters['n_DLCs']):
            name = 'subcycle_%d' % j
            par_sys_name= "parallel.{:s}.".format(name)
            prob.model.add_constraint(par_sys_name+'UC1.sig',  lower=10.e6, upper=parameters['UCmat'].fp/parameters['PSF_mat'], ref=1000.e6) #This is yield constraint and no slacke.g., 10 MPa, to avoid slack conditions
    ...
    for j in range(parameters['n_DLCs']):
        name = 'subcycle_%d' % j
        par_sys_name= "parallel.{:s}.".format(name)
        prob.set_val(par_sys_name+'LegCableBal.uvw_legtip_guess',prob.get_val(par_sys_name+'uvw_legtip')) #Assign new guesses
    ...
    if len(opti):
        ...
        print('Optimized cable mass ={:5.3f} kg'.format(*prob.get_val('UC1LC1_mass.cables_mass')))
        #print('D_c=',design_vars['D_Cs.D_C'])
        #print('sig0_c=',design_vars['sig0_Cs.sig0_C'])
        print_results(prob, n_DLCs=parameters['n_DLCs'], post_opt=True, bline_yaml=None)
        #Final cable configuration
        LC1_xyz = np.copy(parameters['LC1_xyz0'])
        LC1_xyz[1,:] = prob.get_val('parallel.subcycle_1.LC1.Bp')
        LC2_xyz = np.copy(LC1_xyz)*np.array([[1.,-1.,1.],[1.,-1.,1.]])
        LC2_xyz[1,:] = prob.get_val('parallel.subcycle_1.LC2.Bp')
        UC1_xyz = np.copy(parameters['UC1_xyz0'])
        UC1_xyz[1,:] = prob.get_val('parallel.subcycle_1.UC1.Bp')
        UC2_xyz = np.copy(UC1_xyz)*np.array([[1.,-1.,1.],[1.,-1.,1.]])
        UC2_xyz[1,:] = prob.get_val('parallel.subcycle_1.UC2.Bp')
...
def print_results(prob,*args, n_DLCs=2, post_opt=True, bline_yaml=None):
    ...
    for j in range(n_DLCs): 
        ...
        print(('Fxyz_tip= '+3*'{:5.3f} ' +'kN; Moments= '+3*'{:5.3f} ' +'kN*m \n').\
                     format(*prob.model.options['parameters']['Fxyz_tip'][j,:]/1.e3))
        print('_______________________________________________________\n')

        print(('LC1.Nxyz= '+3*'{:5.3f} ' +'kN; UC1.Nxyz= '+3*'{:5.3f} ' +'kN \n').\
                    format(*prob.get_val(par_sys_name+'LC1.Nxyz')/1.e3,*prob.get_val(par_sys_name+'UC1.Nxyz')/1.e3))
    
        print(('LC2.Nxyz= '+3*'{:5.3f} ' +'kN; UC2.Nxyz= '+3*'{:5.3f} ' +'kN\n').\
                    format(*prob.get_val(par_sys_name+'LC2.Nxyz')/1.e3,*prob.get_val(par_sys_name+'UC2.Nxyz')/1.e3))
    
        print(('LC1.sig= {:5.3f} ' +'MPa; UC1.sig= {:5.3f} ' +'MPa \n').\
                    format(*prob.get_val(par_sys_name+'LC1.sig')/1.e6,*prob.get_val(par_sys_name+'UC1.sig')/1.e6))
    
        print(('LC2.sig= {:5.3f} ' +'MPa; UC2.sig= {:5.3f} ' +'MPa \n').\
                    format(*prob.get_val(par_sys_name+'LC2.sig')/1.e6,*prob.get_val(par_sys_name+'UC2.sig')/1.e6))
    
        print('Total cable mass (1 UC + 1 LC)= {:.3f} kg \n'.format(*prob.get_val('UC1LC1_mass.cables_mass')))
    
        print(('Leg tip deflections x,y,z =  '+3*'{:5.3f} ' +'m \n').format(*prob.get_val(par_sys_name+'uvw_legtip')))
        print(('Leg tip twist             =  {:5.3f} deg \n').format(np.rad2deg(*prob.get_val(par_sys_name+'tht_legtip'))))
        
        print(('Leg root reactions x,y,z =  '+3*'{:5.3f} ' +'kN \n').format(*prob.get_val(par_sys_name+'LegCableBal.XYZ0')/1.e3))
        print('_______________________________________________________\n')
        print('_______________________________________________________\n')
...
if __name__ == '__main__':
   ...
   #Fxyz_tip_def = np.array(([0.0, 100.e3, 6.e6, 0.,0.,0.],[0.0, 2.e6, 6.e6, 0.,0.,0.]))  # [n_loadcases X 6]
    Fxyz_tip_def = np.array(([0.0, 0.0, 1.3473e7, 0.,0.,0.],[0.0, 1.4787e7, 7.72065e6, 0.,0.,0.]))  # [n_loadcases X 6] ,[0.0, 0.0, 623.7e3, 0.,0.,0.]

    D_L_def=3. # [m] leg OD
    L_L0_def=35. # [m] leg length
    L_Lxyz0_def = np.array([L_L0_def,0.,0.]) #components of leg vector root to tip in global CS
    M_L_def=202.73e3 # [kg] leg mass
    ...
    D_c_def = [405.47e-3,211.78e-3] #lower,upper cable ODs [m] Maxs
    #sig0_c_def = np.array([540.E6, 950.e6]) #lower, upper cable sigma0  constraints[Pa]
    sig0_c_def = np.array([540.29E6, 540.29e6]) #lower, upper cable sigma0  constraints[Pa] Max's
